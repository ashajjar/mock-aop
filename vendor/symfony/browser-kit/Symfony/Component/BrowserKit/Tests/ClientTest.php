...
class ClientTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testGetRequest()
	{
		...
		$this->assertEquals('http://example.com/', $client->getRequest()
			->getUri(), '->getCrawler() returns the Request of the last request');
	}

	public function testGetRequestWithIpAsHost()
	{
		$client = new TestClient();
		$client->request('GET', 'https://example.com/foo', array(), array(), array(
			'HTTP_HOST' => '127.0.0.1'
		));
		
		$this->assertEquals('https://127.0.0.1/foo', $client->getRequest()
			->getUri());
	}
	...
	public function testGetResponse()
	{
		...
		$this->assertEquals('foo', $client->getResponse()
	...
	public function testGetContent()
	{
		...
		$this->assertEquals($json, $client->getRequest()
			->getContent());
	}
	...
	public function testRequestURIConversion()
	{
		...
		$this->assertEquals('http://localhost/foo', $client->getRequest()
			->getUri(), '->request() converts the URI to an absolute one');
		
		$client = new TestClient();
		$client->request('GET', 'http://www.example.com');
		$this->assertEquals('http://www.example.com', $client->getRequest()
			->getUri(), '->request() does not change absolute URIs');
		
		$client = new TestClient();
		$client->request('GET', 'http://www.example.com/');
		$client->request('GET', '/foo');
		$this->assertEquals('http://www.example.com/foo', $client->getRequest()
		...
		$this->assertEquals('http://www.example.com/foo#', $client->getRequest()
			->getUri(), '->request() uses the previous request for #');
		$client->request('GET', '#foo');
		$this->assertEquals('http://www.example.com/foo#foo', $client->getRequest()
			->getUri(), '->request() uses the previous request for #');
		
		$client = new TestClient();
		$client->request('GET', 'http://www.example.com/foo/');
		$client->request('GET', 'bar');
		$this->assertEquals('http://www.example.com/foo/bar', $client->getRequest()
			->getUri(), '->request() uses the previous request for relative URLs');
	...
	public function testRequestURIConversionByServerHost()
	{
		...
		$this->assertEquals('http://www.exampl+e.com:8000', $client->getRequest()
	...
	public function testRequestHistory()
	{
		...
		$this->assertEquals('http://www.example.com/foo/bar', $client->getHistory()
			->current()
			->getUri(), '->request() updates the History');
		$this->assertEquals('http://www.example.com/foo/foobar', $client->getHistory()
			->back()
			->getUri(), '->request() updates the History');
	}
	...
	{
		$client = new TestClient();
		$client->setNextResponse(new Response('<html><a href="/foo">foo</a></html>', 200, array(
			'Set-Cookie' => 'foo=bar'
		)));
		$client->request('GET', 'http://www.example.com/foo/foobar');
	...
	public function testRequestSecureCookies()
	{
		...
		$this->assertTrue($client->getCookieJar()
	...
	public function testClickForm()
	{
		...
		$client->click($crawler->filter('input')
			->form());
		
		$this->assertEquals('http://www.example.com/foo', $client->getRequest()
			->getUri(), '->click() Form submit forms');
	}
	...
	public function testSubmit()
	{
		...
		$client->submit($crawler->filter('input')
			->form());
	...
	public function testFollowRedirect()
	{
		...
		$this->assertEquals('http://www.example.com/redirected', $client->getRequest()
		...
		$client = new TestClient();
		$client->setNextResponse(new Response('', 201, array(
			'Location' => 'http://www.example.com/redirected'
		)));
		$client->request('GET', 'http://www.example.com/foo/foobar');
		
		$this->assertEquals('http://www.example.com/foo/foobar', $client->getRequest()
	...
	public function testFollowRelativeRedirect()
	{
		...
		$client->request('GET', 'http://www.example.com/foo/foobar');
		$this->assertEquals('http://www.example.com/redirected:1234', $client->getRequest()
	...
	public function testFollowRedirectWithMaxRedirects()
	{
		...
		$this->assertEquals('https://www.example.org/', $client->getRequest()
		...
		$client->request('POST', 'http://www.example.com/foo/foobar', array(
			'name' => 'bar'
		));
		
		$this->assertEquals('get', $client->getRequest()
			->getMethod(), '->followRedirect() uses a get for 302');
		$this->assertEquals(array(), $client->getRequest()
	...
	public function testFollowRedirectWithHeaders()
	{
		...
		$this->assertEquals($headers, $client->getRequest()
			->getServer());
	...
	public function testBack()
	{
		...
		$this->assertArrayHasKey('foo', $client->getRequest()
			->getParameters(), '->back() keeps parameters');
		$this->assertArrayHasKey('myfile.foo', $client->getRequest()
			->getFiles(), '->back() keeps files');
		$this->assertArrayHasKey('X_TEST_FOO', $client->getRequest()
			->getServer(), '->back() keeps $_SERVER');
		$this->assertEquals($content, $client->getRequest()
	...
	public function testRestart()
	{
		...
		$this->assertTrue($client->getHistory()
			->isEmpty(), '->restart() clears the history');
		$this->assertEquals(array(), $client->getCookieJar()
	...
	public function testInsulatedRequests()
	{
		...
		$this->assertEquals('foobar', $client->getResponse()
	...
	public function testSetServerParameterInRequest()
	{
		...
		$this->assertEquals('http://testhost/https/www.example.com', $client->getRequest()
			->getUri());
