...
class CookieJarTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testAll()
	{
		...
		$this->assertEquals(array(
			$cookie1,
			$cookie2
		), $cookieJar->all(), '->all() returns all cookies in the jar');
	}
	...
	public function testUpdateFromResponse()
	{
		...
		$this->assertEquals('foo', $cookieJar->get('foo')
			->getValue(), '->updateFromResponse() updates cookies from a Response objects');
	}
	...
	public function testUpdateFromSetCookie()
	{
		...
		$this->assertEquals('bar', $cookieJar->get('bar')
	...
	public function testCookieExpireWithSameNameButDifferentPaths()
	{
		...
		$this->assertEquals(array(
			'foo' => 'bar2'
		), $cookieJar->allValues('http://example.com/bar'));
	}
	...
	public function testCookieWithSameNameButDifferentPaths()
	{
		...
		$this->assertEquals(array(
			'foo' => 'bar1'
		), $cookieJar->allValues('http://example.com/foo'));
		$this->assertEquals(array(
			'foo' => 'bar2'
		), $cookieJar->allValues('http://example.com/bar'));
	}
	...
	public function testCookieWithSameNameButDifferentDomains()
	{
		...
		$this->assertEquals(array(
			'foo' => 'bar1'
		), $cookieJar->allValues('http://foo.example.com/'));
		$this->assertEquals(array(
			'foo' => 'bar2'
		), $cookieJar->allValues('http://bar.example.com/'));
	}
	...
	public function testCookieWithWildcardDomain()
	{
		...
		$this->assertEquals(array(
			'foo' => 'bar'
		), $cookieJar->allValues('http://www.example.com'));
