...
class ResponseTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testGetHeaders()
	{
		...
		$this->assertEquals(array(
			'foo' => 'bar'
		), $response->getHeaders(), '->getHeaders() returns the headers of the response');
	}
	...
	public function testGetHeader()
	{
		...
		$this->assertEquals(array(
			'foo=bar',
			'bar=foo'
		), $response->getHeader('Set-Cookie', false), '->getHeader() returns all header values if first is false');
