...
class PdoSessionHandlerTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testSessionDestroy()
	{
		...
		$storage->write('foo', 'bar');
		$this->assertCount(1, $this->pdo->query('SELECT * FROM sessions')
			->fetchAll());
		
		$storage->destroy('foo');
		
		$this->assertCount(0, $this->pdo->query('SELECT * FROM sessions')
			->fetchAll());
	}
	...
	public function testSessionGC()
	{
		...
		$this->assertCount(2, $this->pdo->query('SELECT * FROM sessions')
			->fetchAll());
