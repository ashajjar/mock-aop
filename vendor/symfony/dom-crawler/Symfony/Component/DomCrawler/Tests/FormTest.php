...
class FormTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testConstructorHandlesFormAttribute()
	{
		...
		$this->assertSame($dom->getElementsByTagName('form')
			->item(0), $form1->getFormNode(), 'HTML5-compliant form attribute handled incorrectly');
		...
		$this->assertSame($dom->getElementsByTagName('form')
			->item(1), $form2->getFormNode(), 'HTML5-compliant form attribute handled incorrectly');
	}
	...
	public function testMultiValuedFields()
	{
		...
		$this->assertEquals($form->get('foo[2]')
			->getValue(), 'foo');
		$this->assertEquals($form->get('foo[3]')
			->getValue(), 'foo');
		$this->assertEquals($form->get('bar[foo][0]')
			->getValue(), 'foo');
		$this->assertEquals($form->get('bar[foo][foobar]')
			->getValue(), 'foo');
	...
	public function testGetFormNode()
	{
		...
		$this->assertSame($dom->getElementsByTagName('form')
			->item(0), $form->getFormNode(), '->getFormNode() returns the form node associated with this form');
	}
	...
	public function testFormRegistrySetValues()
	{
		...
		$f2->expects($this->exactly(2))
			->method('setValue')
			->with(2);
		
		$f3->expects($this->exactly(2))
			->method('setValue')
			->with(3);
		
		$fbb->expects($this->exactly(2))
			->method('setValue')
			->with('fbb');
	...
	{
		$field = $this->getMockBuilder('Symfony\\Component\\DomCrawler\\Field\\FormField')
			->setMethods(array(
			'getName',
			'getValue',
			'setValue',
