...
class CrawlerTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testAdd()
	{
		...
		$this->assertEquals('foo', $crawler->filterXPath('//div')
		...
		$this->assertEquals('Foo', $crawler->filterXPath('//body')
	...
	public function testAddHtmlContent()
	{
		...
		$this->assertEquals('http://symfony.com', $crawler->filterXPath('//base')
			->attr('href'), '->addHtmlContent() adds nodes from an HTML string');
		$this->assertEquals('http://symfony.com/contact', $crawler->filterXPath('//a')
			->link()
	...
	public function testAddHtmlContentCharset()
	{
		...
		$this->assertEquals('Tiếng Việt', $crawler->filterXPath('//div')
			->text());
	}
	...
	public function testAddHtmlContentUnsupportedCharset()
	{
		...
		$this->assertEquals('Žťčýů', $crawler->filterXPath('//p')
			->text());
	}
	...
	public function testAddHtmlContentCharsetGbk()
	{
		...
		$this->assertEquals('中文', $crawler->filterXPath('//p')
			->text());
	}
...
, 'UTF-8');
	...
	public function testAddContent()
	{
		...
		$this->assertEquals('中文', $crawler->filterXPath('//span')
		...
		$this->assertEquals('日本語', $crawler->filterXPath('//body')
	...
	{
		$crawler = new Crawler();
		$crawler->addNode($this->createNodeList()
			->item(0));
	...
	public function testEq()
	{
		...
		$this->assertEquals('Two', $crawler->eq(1)
	...
	{
		$this->assertEquals('One', $this->createTestCrawler()
			->filterXPath('//li')
	...
	{
		$this->assertEquals('<img alt="Bar">', $this->createTestCrawler()
			->filterXPath('//a[5]')
			->html());
		$this->assertEquals('<input type="text" value="TextValue" name="TextName"><input type="submit" value="FooValue" name="FooName" id="FooId"><input type="button" value="BarValue" name="BarName" id="BarId"><button value="ButtonValue" name="ButtonName" id="ButtonId"></button>', trim($this->createTestCrawler()
	...
	public function testExtract()
	{
		...
		$this->assertEquals(array(), $this->createTestCrawler()
			->filterXPath('//ol')
	...
	public function testFilterXpathComplexQueries()
	{
		...
		$this->assertCount(4, $crawler->filterXPath('//form')
		...
		$this->assertCount(1, $crawler->filterXPath('//div[@id="parent"]')
			->filterXPath('./div'), 'A child selection finds only the current div');
		$this->assertCount(3, $crawler->filterXPath('//div[@id="parent"]')
	...
	public function testFilterXPath()
	{
		...
		$crawler = $this->createTestCrawler();
		$this->assertCount(3, $crawler->filterXPath('//body')
			->filterXPath('//button')
	...
	{
		$this->assertCount(3, $this->createTestCrawler()
	...
	public function testFilterWithDefaultNamespaceOnly()
	{
		...
		$this->assertEquals(2, $crawler->filter('url')
			->count());
	}
	...
	public function testSelectButton()
	{
		...
		$this->assertEquals(1, $crawler->selectButton('FooValue')
			->count(), '->selectButton() selects buttons');
		$this->assertEquals(1, $crawler->selectButton('FooName')
			->count(), '->selectButton() selects buttons');
		$this->assertEquals(1, $crawler->selectButton('FooId')
			->count(), '->selectButton() selects buttons');
		
		$this->assertEquals(1, $crawler->selectButton('BarValue')
			->count(), '->selectButton() selects buttons');
		$this->assertEquals(1, $crawler->selectButton('BarName')
			->count(), '->selectButton() selects buttons');
		$this->assertEquals(1, $crawler->selectButton('BarId')
			->count(), '->selectButton() selects buttons');
		
		$this->assertEquals(1, $crawler->selectButton('FooBarValue')
			->count(), '->selectButton() selects buttons with form attribute too');
		$this->assertEquals(1, $crawler->selectButton('FooBarName')
			->count(), '->selectButton() selects buttons with form attribute too');
	}
...
HTML;
		...
		$this->assertCount(1, $crawler->selectLink('Login')
			->selectLink('Login'));
		$this->assertCount(1, $crawler->selectButton('Submit')
			->selectButton('Submit'));
	}
	...
	public function testChaining()
	{
		...
		$this->assertEquals('a', $crawler->filterXPath('//div')
			->filterXPath('div')
			->filterXPath('div')
			->attr('name'));
	}
	...
	public function testForm()
	{
		...
		$this->assertEquals($crawler->form()
			->getFormNode()
			->getAttribute('id'), $crawler2->form()
			->getFormNode()
	...
	public function testLast()
	{
		...
		$this->assertEquals('Three', $crawler->last()
			->text());
	}
	...
	public function testFirst()
	{
		...
		$this->assertEquals('One', $crawler->first()
			->text());
	}
	...
	public function testSiblings()
	{
		...
		$this->assertEquals('One', $nodes->eq(0)
			->text());
		$this->assertEquals('Three', $nodes->eq(1)
			->text());
		
		$nodes = $this->createTestCrawler()
			->filterXPath('//li')
			->eq(0)
			->siblings();
		$this->assertEquals(2, $nodes->count());
		$this->assertEquals('Two', $nodes->eq(0)
			->text());
		$this->assertEquals('Three', $nodes->eq(1)
			->text());
	...
	public function testNextAll()
	{
		...
		$this->assertEquals('Three', $nodes->eq(0)
			->text());
	...
	public function testChildren()
	{
		...
		$this->assertEquals('One', $nodes->eq(0)
			->text());
		$this->assertEquals('Two', $nodes->eq(1)
			->text());
		$this->assertEquals('Three', $nodes->eq(2)
			->text());
	...
	public function testBaseTag($baseValue, $linkValue, $expectedUri, $currentUri = null, $description = null)
	{
		...
		$this->assertEquals($expectedUri, $crawler->filterXPath('//a')
			->link()
			->getUri(), $description);
	}
	...
	public function testBaseTagWithForm($baseValue, $actionValue, $expectedUri, $currentUri = null, $description = null)
	{
		...
		$this->assertEquals($expectedUri, $crawler->filterXPath('//button')
			->form()
			->getUri(), $description);
	}
