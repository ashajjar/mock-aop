...
class IsTest extends TestFixture
{
	...
	{
		$this->assertFalse(Carbon::now()->endOfDay()
			->isYesterday());
	}

	public function testIsYesterdayFalseWith2Days()
	{
		$this->assertFalse(Carbon::now()->subDays(2)
			->startOfDay()
			->isYesterday());
	}
	...
	{
		$this->assertFalse(Carbon::now()->subDay()
			->endOfDay()
			->isToday());
	}
	...
	{
		$this->assertFalse(Carbon::now()->addDay()
			->startOfDay()
			->isToday());
	}
	...
	{
		$this->assertTrue(Carbon::now()->addDay()
			->isTomorrow());
	}
	...
	{
		$this->assertFalse(Carbon::now()->addDays(2)
			->startOfDay()
			->isTomorrow());
	}
	...
	{
		$this->assertTrue(Carbon::now()->addSecond()
			->isFuture());
	}
	...
	{
		$this->assertFalse(Carbon::now()->addSecond()
			->isPast());
