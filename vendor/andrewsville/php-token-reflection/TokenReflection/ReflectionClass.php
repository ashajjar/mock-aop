...
class ReflectionClass extends ReflectionElement implements IReflectionClass
{
	 ...
	 *        	Class name or reflection object
	 * @return boolean
	 ...
	 *        	Interface name or reflection object
	 * @return boolean
	...
	public function getInterfaceNames()
	{
		...
		foreach ($this->interfaces as $interfaceName) {
			...
			foreach (array_reverse($this->getBroker()
				->getClass($interfaceName)
				->getInterfaceNames()) as $parentInterfaceName) {
	 ...
	 *
	 * @param \ReflectionClass|\TokenReflection\IReflectionClass|string $trait
	 *        	Trait reflection or name
	 * @return boolean
	...
	public function __toString()
	{
		...
		if (count($interfaceNames) > 0) {
			$implements = sprintf(' %s %s', $this->isInterface() ? 'extends' : 'implements', implode(', ', $interfaceNames));
		}
