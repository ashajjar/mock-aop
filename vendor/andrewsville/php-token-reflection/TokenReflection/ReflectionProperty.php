...
class ReflectionProperty extends ReflectionElement implements IReflectionProperty
{
	...
	{
		return sprintf("Property [ %s%s%s%s%s\$%s ]\n", $this->isStatic() ? '' : '<default> ', $this->isPublic() ? 'public ' : '', $this->isPrivate() ? 'private ' : '', $this->isProtected() ? 'protected ' : '', $this->isStatic() ? 'static ' : '', $this->getName());
	}
	 ...
	 *        	If the property should be accessible.
	 */
	...
	protected function parse(Stream $tokenStream, IReflection $parent)
	{
		...
		return $this->parseName($tokenStream)->parseDefaultValue($tokenStream);
	}
