...
class ReflectionAnnotation
{
	 ...
	 * Constructor.
	 *
	 * @param \TokenReflection\ReflectionBase $reflection
	 *        	Parent reflection object
	 * @param string|boolean $docComment
	 *        	Docblock definition
	 */
	...
	public function setTemplates(array $templates)
	{
		foreach ($templates as $template) {
			if (! $template instanceof ReflectionAnnotation) {
				throw new Exception\RuntimeException(sprintf('All templates have to be instances of \\TokenReflection\\ReflectionAnnotation; %s given.', is_object($template) ? get_class($template) : gettype($template)), Exception\RuntimeException::INVALID_ARGUMENT, $this->reflection);
			}
		}
	...
	private function parse()
	{
		...
		if (false !== $this->docComment) {
			...
			$docblock = trim(preg_replace(array(
	...
	private function inheritAnnotations()
	{
		...
		$parents = array_filter(array_merge(array(
