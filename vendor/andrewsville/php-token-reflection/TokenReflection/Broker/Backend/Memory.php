...
class Memory implements Broker\Backend
{
	...
	public function getClass($className)
	{
		...
		$className = ltrim($className, '\\');
		try {
			$ns = $this->getNamespace(($boundary = strrpos($className, '\\')) ? 
	...
	public function getFunction($functionName)
	{
		...
		$functionName = ltrim($functionName, '\\');
		try {
			$ns = $this->getNamespace(($boundary = strrpos($functionName, '\\')) ? 
	 ...
	 *        	Token stream
	 * @param \TokenReflection\ReflectionFile $file
	 *        	File reflection object
