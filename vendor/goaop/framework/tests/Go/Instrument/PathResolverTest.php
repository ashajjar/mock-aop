...
class PathResolverTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testRealpathWorkingCorrectly($path, $expected)
	{
		...
		list ($pathScheme, $localPath) = isset($components[1]) ? $components : array(
			null,
			$components[0]
		);
		
		// resolve path parts (single dot, double dot and double delimiters)
		$localPath = str_replace(array(
			'/',
			'\\'
	...
	public function realpathExamples()
	{
		...
		return array(
			...
			array(
				'/some/absolute/file/../points/',
				'/some/absolute/points/'
			),
			array(
				'/some/./point.php',
				'/some/point.php'
			...
			array(
				'relative/to/the/dir',
				"$curDir/relative/to/the/dir"
			),
			array(
				'../relative/filename',
				"$parent/relative/filename"
			),
			array(
				'./point/file',
				"$curDir/point/file"
			...
			array(
				'http://localhost/some/../relative.file',
				'http://localhost/relative.file'
			),
			
			array(
				'phar://go.phar/some/path',
				'phar://go.phar/some/path'
			),
			array(
				'phar://go.phar/some/../relative.file',
				'phar://go.phar/relative.file'
			)
		);
	}
}
