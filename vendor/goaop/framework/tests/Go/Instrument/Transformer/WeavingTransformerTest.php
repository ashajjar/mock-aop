...
class WeavingTransformerTest extends \PHPUnit_Framework_TestCase
{
	...
	public function setUp()
	{
		...
		$loader = $this->getMock('Go\Core\AspectLoader', array(), array(
			$container,
			$reader
		));
		...
		$this->kernel = $this->getKernelMock(array(
			'appDir' => dirname(__DIR__),
			'cacheDir' => null,
			'includePaths' => array(),
			'excludePaths' => array()
		), $container);
		$this->transformer = new WeavingTransformer($this->kernel, $this->broker, $this->adviceMatcher, $this->getMock('Go\Instrument\ClassLoading\CachePathManager', array(), array(
			$this->kernel
		)), $loader);
	...
	public function testTransformerWithIncludePaths()
	{
		...
		$this->transformer = new WeavingTransformer($this->getKernelMock(array(
			'appDir' => dirname(__DIR__),
			'includePaths' => array(
				__DIR__
			),
			'excludePaths' => array()
		), $container), $this->broker, $this->adviceMatcher, $this->getMock('Go\Instrument\ClassLoading\CachePathManager', array(), array(
			$this->kernel
		)), $loader);
		$this->metadata->source = $this->loadTest('class');
		$this->transformer->transform($this->metadata);
	...
	public function testTransformerWithStaticLsbFeature()
	{
		$this->kernel->expects($this->any())
			->method('hasFeature')
			->will($this->returnCallback(function ($feature)
		{
	...
	{
		return strtr(preg_replace('/\s+$/m', '', $value), array(
			"\r\n" => PHP_EOL,
			"\n" => PHP_EOL
		));
	}
	...
	{
		$mock = $this->getMockForAbstractClass('Go\Core\AspectKernel', array(), '', false, true, true, array(
			'getOptions',
			'getContainer',
			'hasFeature'
		));
	...
	{
		$mock = $this->getMock('Go\Core\AdviceMatcher', array(
			'getAdvicesForClass'
		), array(), '', false);
		$mock->expects($this->any())
			->method('getAdvicesForClass')
			->will($this->returnCallback(function (\TokenReflection\ReflectionClass $refClass)
		{
			$advices = array();
