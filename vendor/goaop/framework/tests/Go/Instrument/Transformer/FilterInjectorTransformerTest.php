...
class FilterInjectorTransformerTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		if (! self::$transformer) {
			$kernelMock = $this->getKernelMock(array(
				'cacheDir' => null,
				'appDir' => '',
				'debug' => false,
				'features' => 0
			), $this->getMock('Go\Core\GoAspectContainer'));
			self::$transformer = new FilterInjectorTransformer($kernelMock, 'unit.test', $this->getMock('Go\Instrument\ClassLoading\CachePathManager', array(), array(
				$kernelMock
			)));
		}
	...
	public function testCanRewriteRelativePathsWithFilter()
	{
		...
		$output = FilterInjectorTransformer::PHP_FILTER_READ . 'unit.test/resource=' . stream_resolve_include_path('_files/class.php');
		$this->assertEquals($output, $this->metadata->source);
	}
