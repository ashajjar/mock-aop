...
abstract class AspectKernel
{
	 ...
	 *
	 * @param array $options
	 *        	Associative array of options for kernel
	 */
	...
	protected function registerTransformers()
	{
		...
		$sourceTransformers = function () use($filterInjector, $magicTransformer, $aspectKernel, $cacheManager)
		{
			...
			$transformers[] = new WeavingTransformer($aspectKernel, new TokenReflection\Broker(new CleanableMemory()), $aspectContainer->get('aspect.advice_matcher'), $cacheManager, $aspectContainer->get('aspect.cached.loader'));
