...
class GoAspectContainer extends Container implements AspectContainer
{
	...
	{
		$this->set("pointcut.{$id}", $pointcut, array(
			'pointcut'
		));
	}
	...
	{
		$this->set("advisor.{$id}", $advisor, array(
			'advisor'
		));
	}
	 ...
	 *
	 * @param Aop\Aspect $aspect
	 *        	Instance of concrete aspect
	 */
	public function registerAspect(Aop\Aspect $aspect)
	{
		$refAspect = new ReflectionClass($aspect);
		$this->set("aspect.{$refAspect->name}", $aspect, array(
			'aspect'
		));
