...
class FilterInjectorTransformer implements SourceTransformer
{
	 ...
	 *        	Kernel to take configuration from
	 * @param string $filterName
	 *        	Name of the filter to inject
	 * @param CachePathManager $cacheManager
	 *        	Manager for cache files
	 */
	 ...
	 *
	 * @param AspectKernel $kernel
	 *        	Kernel to use for configuration
	 * @param string $filterName
	 *        	Name of the filter to inject
	 * @param CachePathManager $cacheManager
	 *        	Cache manager
	 ...
	 *        	Initial resource to include
	 * @param string $originalDir
	 *        	Path to the directory from where include was called for resolving relative resources
	 *        	
	...
	public function transform(StreamMetaData $metadata)
	{
		...
		foreach ($tokenStream as $token) {
			...
			list ($token, $value) = (array) $token + array(
				1 => $token
			);
