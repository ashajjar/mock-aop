...
class WeavingTransformer extends BaseSourceTransformer
{
	 ...
	 * @param Broker $broker
	 *        	Instance of reflection broker to use
	 * @param AdviceMatcher $adviceMatcher
	 ...
	 *        	Loader for aspects
	 */
	public function __construct(AspectKernel $kernel, Broker $broker, AdviceMatcher $adviceMatcher, CachePathManager $cachePathManager, AspectLoader $loader)
	{
	 ...
	 *        	Instance of class to analyze
	 * @param integer $lineOffset
	 *        	Current offset, will be updated to store the last position
	 *        	
	...
	private function processSingleClass(array $advisors, StreamMetaData $metadata, ParsedClass $class, &$lineOffset)
	{
		...
		$child = ($this->kernel->hasFeature(Features::USE_TRAIT) && $class->isTrait()) ? new TraitProxy($class, $advices, $useStatic) : new ClassProxy($class, $advices, $useStatic);
		...
		// Add child to source
		$tokenCount = $class->getBroker()
			->getFileTokens($class->getFileName())
			->count();
		if ($tokenCount - $class->getEndPosition() < 3) {
			...
			$dataArray = array_merge($currentClassArray, array(
				''
	 ...
	 *
	 * @param ParsedClass $class
	 *        	Instance of class reflection
	 * @param string $source
	 *        	Source code
	 * @param string $newParentName
	 *        	New name for the parent class
	 *        	
	 ...
	 * @param StreamMetaData $metadata
	 *        	Source stream information
	 * @param ParsedFileNamespace $namespace
	 ...
	 *
	 * @param array $unloadedAspects
	 *        	List of unloaded aspects
	 */
