 ...
 *
 * @see https://github.com/php/php-src/blob/master/Zend/zend_language_parser.y new_expr:
 *      T_NEW
...
class ConstructorExecutionTransformer implements SourceTransformer
{
	...
	public function transform(StreamMetaData $metadata = null)
	{
		...
		foreach ($tokenStream as $index => $token) {
			list ($token, $tokenValue) = (array) $token + array(
				1 => $token
			);
	 ...
	 *        	Name of the class to construct
	 *        	
