...
class PointcutGrammar extends Grammar
{
	...
	public function __construct(AspectContainer $container = null, RawAnnotationReader $annotationReader = null)
	{
		...
		$this('pointcutExpression')->is('pointcutExpression', '||', 'conjugatedExpression')
		...
		$this('conjugatedExpression')->is('conjugatedExpression', '&&', 'negatedExpression')
			->call(function ($first, $_0, $second)
		{
		...
		$this('negatedExpression')->is('!', 'brakedExpression')
		...
		$this('singlePointcut')->is('accessPointcut')
		...
		$this('executionPointcut')->is('execution', '(', 'methodExecutionReference', ')')
		...
		is('dynamic', '(', 'memberReference', '(', 'argumentList', ')', ')')->call(function ($_0, $_1, ClassMemberReference $reference)
		{
		...
		$this('classFilter')->is('namespacePattern')
		...
		$this('namespaceName')->is('namePart')
			->call($stringConverter)
			->is('namespaceName', 'nsSeparator', 'namePart')
			->call($stringConverter);
		
		$this('memberModifiers')->is('memberModifier', '|', 'memberModifiers')
