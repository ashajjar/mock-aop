...
EOT
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if (! class_exists($class)) {
			$output->writeln(array(
				$formatter->formatBlock(sprintf('The class "%s" could not be found.', $class), 'error', true)
			));
		...
		try {
			...
			if ($conflicts) {
				...
				$output->writeln(sprintf("<info><comment>%d</comment> conflicts in total", count($conflicts)));
		...
		if ($input->getOption('dfa')) {
			...
			if ($input->getOption('state') === null) {
				...
				if (! $automaton->hasState($state)) {
					$output->writeln(array(
						$formatter->formatBlock(sprintf('The automaton has no state #%d', $state), 'error', true)
					));
					
					return 1;
				}
				
				$output->writeln(sprintf('<info>Exporting the DFA state <comment>%d</comment>...', $state));
	...
	{
		$type = $conflict['resolution'] === Grammar::SHIFT ? 'shift/reduce' : 'reduce/reduce';
		
		return sprintf("<info>Resolved a <comment>%s</comment> conflict in state <comment>%d</comment> on lookahead <comment>%s</comment></info>", $type, $conflict['state'], $conflict['lookahead']);
	}
}
