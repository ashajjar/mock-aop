...
EOT;
	 ...
	 * @param \Dissect\Parser\Rule $firstRule
	 *        	The first conflicting grammar rule.
	 * @param \Dissect\Parser\Rule $secondRule
	...
	public function __construct($state, Rule $firstRule, Rule $secondRule, $lookahead, Automaton $automaton)
	{
		...
		parent::__construct(sprintf(self::MESSAGE, $firstRule->getNumber(), $firstRule->getName(), empty($components1) ? '/* empty */' : implode(' ', $components1), $secondRule->getNumber(), $secondRule->getName(), empty($components2) ? '/* empty */' : implode(' ', $components2), $lookahead, $state), $state, $automaton);
