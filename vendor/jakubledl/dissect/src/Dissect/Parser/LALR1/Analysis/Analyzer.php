...
class Analyzer
{
	 ...
	 *
	 * @param \Dissect\Parser\Grammar $grammar
		...
		while (! $queue->isEmpty()) {
					...
					if (array_key_exists($component, $groupedRules)) {
						...
						foreach ($cs as $i => $c) {
							if (! array_key_exists($c, $groupedRules)) {
								...
								$lookahead = Util::union($lookahead, array(
									$c
								));
			...
			foreach ($groupedItems as $thisComponent => $theseItems) {
				...
				foreach ($theseItems as $thisItem) {
					...
					$currentOrigin[$thisItem->getRule()->getNumber()][] = $thisItem->getDotIndex();
				}
				...
				foreach ($origins as $number => $map) {
					...
					foreach ($currentOrigin as $ruleNum => $positions) {
						...
						if (count($currentOrigin) !== count($map) || ! isset($map[$ruleNum]) || 
				...
				if ($n === null) {
					...
					foreach ($theseItems as $thisItem) {
						$thisItem->connect($nextState->get($thisItem->getRule()
							->getNumber(), $thisItem->getDotIndex() + 1));
					}
				}
			}
		}
