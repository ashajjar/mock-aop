...
class AnalyzerTest extends PHPUnit_Framework_TestCase
{
	...
	public function lookaheadShouldBeCorrectlyPumped()
	{
		...
		), $automaton->getState(1)
			->get(0, 1)
			->getLookahead());
		
		$this->assertEquals(array(
			'b'
		), $automaton->getState(3)
			->get(2, 1)
			->getLookahead());
		
		$this->assertEquals(array(
			'd'
		), $automaton->getState(4)
			->get(4, 0)
			->getLookahead());
		
		$this->assertEquals(array(
			'd'
		), $automaton->getState(5)
			->get(3, 1)
			->getLookahead());
		
		$this->assertEquals(array(
			Parser::EOF_TOKEN_TYPE
		), $automaton->getState(7)
			->get(1, 4)
			->getLookahead());
		
		$this->assertEquals(array(
			Parser::EOF_TOKEN_TYPE
		), $automaton->getState(8)
			->get(5, 1)
			->getLookahead());
	}
	...
	public function unexpectedConflictsShouldThrowAnException()
	{
		...
		$grammar('S')->is('a', 'b', 'C', 'd')->is('a', 'b', 'E', 'd');
		...
		try {
			...
			$this->assertEquals(3, $e->getFirstRule()
				->getNumber());
			$this->assertEquals(4, $e->getSecondRule()
				->getNumber());
		}
	}
