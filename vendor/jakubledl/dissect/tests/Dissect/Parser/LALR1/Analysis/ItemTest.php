...
class ItemTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$item = new Item(new Rule(1, 'A', array(
			'a',
			'b',
			'c'
		)), 1);
	...
	{
		$item1 = new Item(new Rule(1, 'A', array(
			'a',
			'b',
			'c'
		)), 1);
		$item2 = new Item(new Rule(1, 'A', array(
			'a',
			'b',
			'c'
		)), 2);
	...
	public function itemShouldPumpTheSameLookaheadOnlyOnce()
	{
		...
		$item2 = $this->getMock('Dissect\\Parser\\LALR1\\Analysis\\Item', array(
			'pump'
		), array(
			new Rule(1, 'A', array(
				'a',
	...
	public function getUnrecognizedComponentsShouldReturnAllComponentAfterTheDottedOne()
	{
		...
		$this->assertEquals(array(
			'c'
		), $item->getUnrecognizedComponents());
	}
}
