...
class SimpleLexerTest extends PHPUnit_Framework_TestCase
{
	...
	public function simpleLexerShouldWalkThroughTheRecognizers()
	{
		...
		$this->assertEquals('(', $stream->get(1)
			->getType());
		$this->assertEquals(1, $stream->get(3)
			->getLine());
		$this->assertEquals('C', $stream->get(4)
			->getType());
	}
	...
	public function simpleLexerShouldReturnTheBestMatch()
	{
		...
		$this->assertEquals('CLASS', $stream->getCurrentToken()
			->getType());
		$this->assertEquals('WORD', $stream->lookAhead(1)
			->getType());
	}
}
