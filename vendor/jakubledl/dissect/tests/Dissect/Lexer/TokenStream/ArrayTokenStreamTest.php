...
class ArrayTokenStreamTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$this->assertEquals('5', $this->stream->lookAhead(2)
			->getValue());
	}
	...
	{
		$this->assertEquals('3', $this->stream->get(4)
			->getValue());
	}
	...
	{
		$this->stream->move(2);
		$this->assertEquals('5', $this->stream->getCurrentToken()
			->getValue());
	}
	...
	{
		$this->stream->seek(4);
		$this->assertEquals('3', $this->stream->getCurrentToken()
			->getValue());
	}
	...
	{
		$this->stream->next();
		$this->assertEquals('PLUS', $this->stream->getCurrentToken()
			->getType());
