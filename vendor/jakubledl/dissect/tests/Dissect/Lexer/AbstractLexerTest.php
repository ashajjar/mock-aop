...
class AbstractLexerTest extends PHPUnit_Framework_TestCase
{
	...
	public function lexShouldDelegateToExtractTokenUpdatingTheLineAndOffsetAccordingly()
	{
		...
		$this->assertEquals('a', $stream->getCurrentToken()
			->getValue());
		$this->assertEquals(1, $stream->getCurrentToken()
			->getLine());
		$stream->next();
		
		$this->assertEquals('b', $stream->getCurrentToken()
			->getValue());
		$this->assertEquals(1, $stream->getCurrentToken()
			->getLine());
		$stream->next();
		...
		$this->assertEquals('c', $stream->getCurrentToken()
			->getValue());
	...
	public function lexShouldAppendAnEofTokenAutomatically()
	{
		...
		$this->assertEquals(Parser::EOF_TOKEN_TYPE, $stream->getCurrentToken()
			->getType());
		$this->assertEquals(1, $stream->getCurrentToken()
			->getLine());
	}
	...
	public function lexShouldSkipTokensIfToldToDoSo()
	{
		...
		$this->assertNotEquals('e', $stream->get(1)
			->getType());
	}
}
